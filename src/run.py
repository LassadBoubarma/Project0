"""Sequential benchmark runner. Raw responses are retained and never repaired/retried."""
import argparse
import csv
import hashlib
import json
...
FIELDS = [
    'role','model','resolved_model','item_id','abbreviation','expected','output','parsed',
...
def call_model(model, prompt, cfg):
    if model['provider'] == 'gemini':
        # The API key is sent only as a header; it is never written to URLs or saved artefacts.
        url = ('https://generativelanguage.googleapis.com/v1beta/models/' +
               urllib.parse.quote(model['model'], safe='') + ':generateContent')
        payload = {
        ...
        return request_json(
            url, payload,
            {'x-goog-api-key': api_key_for(model)},
            cfg['timeout_seconds']
        )
    ...
    raise ValueError('Unsupported provider: ' + str(model.get('provider')))
...
def decode_response(raw, model):
    if model['provider'] == 'gemini':
        ...
        status = 'refusal' if blocked or reason in {
        ...
        return output, status, {
            'resolved_model': raw.get('modelVersion', model['model']),
            'input_tokens': usage.get('promptTokenCount', ''),
            'output_tokens': usage.get('candidatesTokenCount', 0),
            'thinking_tokens': usage.get('thoughtsTokenCount', 0),
            'cached_tokens': usage.get('cachedContentTokenCount', 0),
            'api_cost_usd': api_cost(usage, model)
        }
...
def fingerprint():
    files = [
        'config.json','data/items.jsonl','data/dev.jsonl','data/word_list.json',
        'data/label_reviews.csv','src/prompt.txt','src/run.py','src/score.py','src/cost.py'
    ]
...
def local_cost_ready(cfg):
    ...
    if any(settings.get(k) is None for k in required):
        return False
    try:
        return all(float(settings[k]) >= 0 for k in required)
...
def hardware_snapshot(local_model_record, ollama_version):
    """Best-effort real machine details; no guessed hardware values."""
    info = {
        'model': local_model_record,
    ...
    if os.name == 'nt':
        ...
        if Path('/proc/meminfo').exists():
            try:
                kb = int(next(line.split()[1] for line in Path('/proc/meminfo').read_text().splitlines()
                              if line.startswith('MemTotal:')))
                info['ram_gb'] = round(kb / 1024 / 1024, 1)
            except (StopIteration, ValueError):
                pass
        gpu = _command_output(['nvidia-smi','--query-gpu=name,memory.total','--format=csv,noheader'])
        info['gpu'] = gpu
    return info
...
def preflight(cfg):
    missing = sorted({
        m.get('api_key_env', 'GEMINI_API_KEY')
        for m in cfg['models'] if m['provider'] != 'ollama' and not api_key_for(m)
    })
    if missing:
        raise ValueError('Missing API key environment variable(s): ' + ', '.join(missing) +
                         '. Follow START_HERE.md; never put keys in config.json.')

    tags = request_json('http://localhost:11434/api/tags', timeout=10)
    local = next(m for m in cfg['models'] if m['provider'] == 'ollama')
    match = next((m for m in tags.get('models', []) if m.get('name') == local['model']), None)
    if not match:
        raise ValueError('Local model missing. Run: ollama pull ' + local['model'])
    version = request_json('http://localhost:11434/api/version', timeout=10)
    return hardware_snapshot(match, version)


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--split', choices=['dev','test'], default='test')
    parser.add_argument('--check', action='store_true', help='Offline dataset/scorer validation; no model calls')
    args = parser.parse_args()

    cfg = read_json(ROOT / 'config.json')
    items, vocab = load_data(args.split)
    ...
    if args.check:
        ...
        print('Local cost assumptions:', 'complete' if local_cost_ready(cfg) else 'PENDING')
        print('No model requests made. This is NOT a benchmark result.')
        return

    if cfg['temperature'] != 0:
        raise ValueError('Teacher requires temperature 0.')
    if args.split == 'test' and not reviews_ready(items):
        raise ValueError('Complete data/label_reviews.csv with two real reviewers per test item before the frozen run.')
    if args.split == 'test' and not local_cost_ready(cfg):
        raise ValueError('Fill the three local_cost assumptions in config.json before the final test; see START_HERE.md.')
    ...
    (destination / 'items.json').write_text(json.dumps(items, indent=2), encoding='utf-8')
    (destination / 'prompts.json').write_text(
        json.dumps([prompt_for(r, vocab) for r in items], indent=2), encoding='utf-8')

    with (destination / 'per_item.csv').open('w', newline='', encoding='utf-8') as f, \
        ...
        for model in cfg['models']:
            for item in items:
                ...
                row.update(
                    role=model['role'], model=model['model'], item_id=item['id'],
                ...
                try:
                    ...
                    row.update(status='timeout' if is_timeout else 'connection_error',
                ...
                rawfile.write(json.dumps({
                    'role': model['role'], 'item_id': item['id'],
                    'response': raw, 'error': row['error']
                }) + '\n')
                rawfile.flush()
    ...
    if args.split == 'dev':
        print('Practice only. Do not include dev outcomes in the final test results.')
